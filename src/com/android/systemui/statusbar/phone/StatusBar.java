...
public class StatusBar extends SystemUI implements DemoMode,
    ...
    // [HTI] For NavigationBar show or not
...
/// M: add for plmn display feature @{
    void attachPlmnPlugin() {
        try {
            if (mStatusBarPlmnPlugin == null) {
                mSystemUIFactoryBase = OpSystemUICustomizationFactoryBase.getOpFactory(mContext);
                mStatusBarPlmnPlugin = mSystemUIFactoryBase.makeStatusBarPlmn(mContext);
                Log.d(TAG, "create plugin mStatusBarPlmnPlugin = " + mStatusBarPlmnPlugin);
            }
        } catch (Exception e) {
            Log.e(TAG, "mStatusBarPlmnPlugin init failed = " + e);
        }


        try {
            mStatusBarPlmnPlugin.addPlmn((LinearLayout) mStatusBarView.findViewById(
                  R.id.status_bar_contents), mContext);
            mStatusBarPlmnPlugin.setPlmnVisibility(View.GONE);
        } catch (Exception e) {
...
///@}
    ...
    private void onLaunchTransitionTimeout() {
        ...
        // M: fix keyguard bouncer no hide bug when turn left affordance icon.
