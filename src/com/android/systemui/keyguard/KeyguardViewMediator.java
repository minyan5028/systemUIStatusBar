...
public class KeyguardViewMediator extends SystemUI {
    ...
    private static final ComponentName REMO6_PASSWORD_ACTIVITY =
            new ComponentName("com.android.settings",
                    "com.android.settings.display.Remo6RemovePasswordActivity");
    ...
    private Remo6KeyguardOverlay mRemo6Overlay;
    ...
    private void setupLocked() {
        ...
        mRemo6Overlay = new Remo6KeyguardOverlay(mContext, mLockPatternUtils,
                new Remo6KeyguardOverlay.Callback() {
                    @Override
                    public void onUnlockSuccess() {
                        mHandler.post(KeyguardViewMediator.this::keyguardDone);
                    }

                    @Override
                    public void onForgotPasswordRequested() {
                        mHandler.post(KeyguardViewMediator.this::launchRemo6ForgotPassword);
                    }
                });
    ...
    private void maybeShowRemo6Overlay() {
        if (mRemo6Overlay == null) {
            return;
        }
        if (shouldShowRemo6Overlay()) {
            mRemo6Overlay.show();
        } else {
            mRemo6Overlay.hide();
        }
    ...
    private void hideRemo6Overlay() {
        if (mRemo6Overlay != null) {
            mRemo6Overlay.hide();
        }
    }

    private boolean shouldShowRemo6Overlay() {
        if (mRemo6Overlay == null || mLockPatternUtils == null) {
            return false;
        }
        int currentUser = KeyguardUpdateMonitor.getCurrentUser();
        return mShowing && !mOccluded && mLockPatternUtils.isSecure(currentUser);
    }

    private void launchRemo6ForgotPassword() {
        hideRemo6Overlay();
        Intent intent = new Intent().setComponent(REMO6_PASSWORD_ACTIVITY);
        ...
        try {
            ...
            Log.w(TAG, "Unable to launch Remo6 password helper", e);
        }
    }
